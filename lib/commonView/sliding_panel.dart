...
class SlidingUpPanelState extends State<SlidingUpPanel> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _ac = AnimationController(
      ...
      value: widget.defaultPanelState == PanelState.close ? 0.0 : 1.0, //set the default panel state (i.e. set initial value of _ac)
    )..addListener(() {
        ...
        widget.body != null
            ? AnimatedBuilder(
              ...
              builder: (context, child) {
                return Positioned(top: widget.parallaxEnabled ? _getParallax() : 0.0, child: child ?? const SizedBox());
              },
              child: SizedBox(height: MediaQuery.of(context).size.height, width: MediaQuery.of(context).size.width, child: widget.body),
            )
            : Container(),
              ...
              child: AnimatedBuilder(
                ...
                builder: (context, child) {
                  return Container(
                    ...
                    decoration:
                        widget.renderPanelSheet
                            ? BoxDecoration(border: widget.border, borderRadius: widget.borderRadius, boxShadow: widget.boxShadow, color: widget.color)
                            : null,
                    child: child,
                  );
                },
                child: Stack(
                  children: <Widget>[
                    ...
                    Positioned(
                      ...
                      child: SizedBox(height: widget.maxHeight, child: widget.panel ?? widget.panelBuilder!(_sc)),
                    ),
  ...
  void _onGestureEnd(Velocity v) {
    ...
    if (v.pixelsPerSecond.dy.abs() >= minFlingVelocity) {
      ...
      if (widget.panelSnapping && widget.snapPoint != null) {
        ...
        _ac.animateTo(_ac.value + visualVelocity * 0.16, duration: const Duration(milliseconds: 410), curve: Curves.decelerate);
      }
  ...
  void _flingPanelToPosition(double targetPos, double velocity) {
    final Simulation simulation = SpringSimulation(SpringDescription.withDampingRatio(mass: 1.0, stiffness: 500.0, ratio: 1.0), _ac.value, targetPos, velocity);
