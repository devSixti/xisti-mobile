...
class LinearPercentIndicatorState extends State<LinearPercentIndicator> with SingleTickerProviderStateMixin, AutomaticKeepAliveClientMixin {
  ...
  void initState() {
    ...
    if (widget.animation) {
      ...
      _animation = Tween(begin: _percent, end: widget.percent).animate(CurvedAnimation(parent: _animationController!, curve: widget.curve))..addListener(() {
        setState(() {
  ...
  Widget build(BuildContext context) {
        ...
        return Container(
          ...
          child: Stack(
            ...
            children: [
              ...
              if (widget.widgetIndicator != null && _indicatorWidth == 0) Opacity(opacity: 0.0, key: _keyIndicator, child: widget.widgetIndicator),
    ...
    return Material(
      ...
      child: Container(
        ...
        child: Row(mainAxisAlignment: widget.alignment, crossAxisAlignment: CrossAxisAlignment.center, children: items),
      ),
    );
  }
...
class _LinearPainter extends CustomPainter {
  ...
  _LinearPainter({
    ...
    _paintLine.color = progress == 0 ? progressColor.withValues(alpha: 0.0) : progressColor;
  }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (linearGradientBackgroundColor != null) {
      ...
      _paintBackground.shader = linearGradientBackgroundColor?.createShader(Rect.fromPoints(Offset.zero, shaderEndPoint), textDirection: TextDirection.ltr);
    }
    ...
    if (isRTL) {
      ...
      linePath.addRRect(RRect.fromRectAndRadius(Rect.fromLTWH(factor, factor, xProgress - correction, size.height - correction), barRadius));
    }
  ...
  Shader _createGradientShaderRightToLeft(Size size, double xProgress) {
    ...
    return linearGradient!.createShader(Rect.fromPoints(Offset(size.width, size.height), shaderEndPoint), textDirection: TextDirection.rtl);
  }
  ...
  Shader _createGradientShaderLeftToRight(Size size, double xProgress) {
    ...
    return linearGradient!.createShader(Rect.fromPoints(Offset.zero, shaderEndPoint), textDirection: TextDirection.ltr);
  }
